...
export const customUrl = defineType({
  ...
  fields: [
    defineField({
      name: "type",
      type: "string",
      options: createRadioListLayout(["internal", "app", "external"]),
  ...
  preview: {
    ...
    prepare({ externalUrl, urlType, internalUrl, appUrl, openInNewTab }) {
      const url =
        urlType === "external"
          ? externalUrl
          : urlType === "internal"
      ...
      return {
        title: `${urlType === "external" ? "External" : urlType === "internal" ? "Internal" : "App"} Link`,
